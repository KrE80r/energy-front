...
def extract_detailed_tariff_periods(detailed_plan_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {**simple_rates, "daily_supply_charge": daily_supply_charge, "detailed_time_blocks": tariff_blocks}
        ...
        return {
            ...
            "raw_plan_data": {
                "eligibility_restrictions": any(
                    "eligibilityRestriction" in contract for contract in plan_data.get("contract", [])
                ),
                "has_discounts": any("discount" in contract for contract in plan_data.get("contract", [])),
...
def extract_all_plans_data(postcode: str) -> Dict[str, List[Dict[str, Any]]]:
    ...
    logger.info(
        f"Successfully extracted {len(extracted_plans)} plans, filtered out {filtered_out} (SR/SRCL/TOUCL), {errors} errors"
    )
    ...
    logger.info(
        f"Plan breakdown: {len(grouped_plans['TOU'])} TOU, {len(grouped_plans['SR'])} SR, {len(grouped_plans['OTHER'])} Other"
    )
